...
function getLanguageFromHeader(request) {
...
__name(getLanguageFromHeader, "getLanguageFromHeader");
async function getUserLanguage(env2, userId) {
  try {
    const result = await env2.DB.prepare(
      "SELECT language FROM users WHERE id = ?"
    ).bind(userId).first();
    if (result && SUPPORTED_LANGUAGES.includes(result.language)) {
      return result.language;
    }
  } catch (error3) {
    console.error("Error fetching user language:", error3);
  }
  return DEFAULT_LANGUAGE;
}
__name(getUserLanguage, "getUserLanguage");
async function getLanguage(request, env2, userId) {
  if (userId) {
    const userLang = await getUserLanguage(env2, userId);
...
var src_default = {
  async fetch(request, env2) {
    ...
    if (url.pathname === "/api/test") {
      const lang = await getLanguage(request, env2);
      const message = lang === "vi" ? "Xin ch\xE0o t\u1EEB Marketer Suite! \u{1F680} (Ng\xF4n ng\u1EEF: Ti\u1EBFng Vi\u1EC7t)" : "Hello from Marketer Suite! \u{1F680} (Language: English)";
      return new Response(
        JSON.stringify({
          success: true,
          language: lang,
...
// .wrangler/tmp/bundle-TRy6yw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TRy6yw/middleware-loader.entry.ts
